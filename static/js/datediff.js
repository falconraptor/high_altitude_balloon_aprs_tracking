...
Date.DateDiff = function (p_Interval, p_Date1, p_Date2, p_FirstDayOfWeek) {
	...
	let dt1 = Date.CDate(p_Date1)
	let dt2 = Date.CDate(p_Date2)
	...
	let iDiffMS = dt2.valueOf() - dt1.valueOf()
	let dtDiff = new Date(iDiffMS)

	// calc various diffs
	let nYears  = dt2.getUTCFullYear() - dt1.getUTCFullYear()
	let nMonths = dt2.getUTCMonth() - dt1.getUTCMonth() + (nYears != 0 ? nYears * 12 : 0)
	let nQuarters = parseInt(nMonths / 3)	//<<-- different than VBScript, which watches rollover not completion

	let nMilliseconds = iDiffMS
	let nSeconds = parseInt(iDiffMS / 1000)
	let nMinutes = parseInt(nSeconds / 60)
	let nHours = parseInt(nMinutes / 60)
	let nDays  = parseInt(nHours / 24)	// <-- now fixed for DST switch days
	let nWeeks = parseInt(nDays / 7)

	if (p_Interval.toLowerCase() == 'ww') {
        // set dates to 1st & last FirstDayOfWeek
        let offset = Date.DatePart("w", dt1, p_FirstDayOfWeek) - 1
        if (offset) dt1.setDate(dt1.getDate() + 7 - offset)
        offset = Date.DatePart("w", dt2, p_FirstDayOfWeek) - 1
        if (offset) dt2.setDate(dt2.getDate() - offset)
        // recurse to "w" with adjusted dates
        let nCalWeeks = Date.DateDiff("w", dt1, dt2) + 1
	}
	// TODO: similar for 'w'?
...
Date.MonthName = function(p_Month, p_Abbreviate){
	...
	let retVal = Date.MonthNames[p_Month]
...
Date.WeekdayName = function (p_Weekday, p_Abbreviate, p_FirstDayOfWeek) {
	...
	let nWeekdayNameIdx = ((p_FirstDayOfWeek - 1 + parseInt(p_Weekday) - 1 + 7) % 7) + 1	// compensate nWeekdayNameIdx for p_FirstDayOfWeek
	let retVal = Date.WeekdayNames[nWeekdayNameIdx]
	if (p_Abbreviate == true) retVal = retVal.substring(0, 3)	// abbr to 3 chars
	return retVal
}
...
Date.Format = function (p_Date, p_Format, p_FirstDayOfWeek, p_firstweekofyear) {
	...
	if (hr == 0) hr = 12
	if (hr > 12) hr -= 12
	let strShortTime = hr + ':' + this.pad(dt.getMinutes()) + ':' + this.pad(dt.getSeconds()) + ' ' + ampm
	let strShortDate = (dt.getMonth() + 1) + '/' + dt.getDate() + '/' + String(dt.getFullYear()).substring(2,4)
	let strLongDate = Date.MonthName(dt.getMonth() + 1) + ' ' + dt.getDate() + ', ' + dt.getFullYear()
	...
	retVal = retVal.replace(new RegExp('yy', 'gi'), String(dt.getFullYear()).substring(2,4))
	...
	let timezone = (tz < 0) ? ('GMT-' + tz / 60) : (tz == 0) ? ('GMT') : ('GMT+' + tz / 60)
